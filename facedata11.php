...
function fbpostswithcomments($id,$collection){
	...
	for($j=0;$j<$cnt;$j++){
	echo "message= ". $json_output["data"][$i]["comments"]["data"][$j]["message"]."<br />";
	echo "name= " . $json_output["data"][$i]["comments"]["data"][$j]["from"]["name"]."<br />";
	echo "created_time= ". $json_output["data"][$i]["comments"]["data"][$j]["created_time"]."<br />";
	echo "manid= ". $json_output["data"][$i]["comments"]["data"][$j]["from"]["id"]."<br />";
	$pageinfo=array(
	       "Name" => $json_output["data"][$i]["comments"]["data"][$j]["from"]["name"],
	       "personid" => $json_output["data"][$i]["comments"]["data"][$j]["from"]["id"],
	        "created_time" => $json_output["data"][$i]["comments"]["data"][$j]["created_time"],
	        "comments" => $json_output["data"][$i]["comments"]["data"][$j]["message"]
	);
    $collection->insert($pageinfo);
   # for($i=0;$i<$cnt1;$i++){
	#	for($j=0;$j<$cnt;$j++){
	#		location($json_output["data"][$i]["comments"]["data"][$j]["from"]["id"],$collection);
	#	}
	#}
...
function location($id,$collection){
	$json_url ='https://graph.facebook.com/v2.6/'.$id.'?fields=location&format=json&access_token=EAAWF1YwTzZBgBACom1a2jMlCTs8O2GyZBkW8DBUQXosqN4acqlR7VkB7XjJwZCb32CPLqHyXW5dxCNt9DeFBvsd7RTmcPf1KCKph9JvaXwtDsLKFSy3PZAVJ9CXFzatHHpZBPDqE2KZCZBsZAfhtbYlhwCiSzPb9LZBgZD';
	$json = file_get_contents($json_url);
	$json_output = json_decode($json, true);
